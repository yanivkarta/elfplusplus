...
def plot_auc_roc_curve(X_train, y_train, X_test, y_test, classifier, y_pred, target_column, ax, title=None):
    ...
    try:
        ...
        for i, color in zip(range(n_classes), colors):
            ...
            auc_score = auc(fpr, tpr)
            ax2.plot(fpr, tpr, lw=2, label='ROC curve of testing class {0} (area = {1:0.2f})' 
...
for classifier in classifiers:
    classifier_name_str = str(classifier)
    classifier_name = classifier_name_str[0:classifier_name_str.find('(')]
    
    print ('[+]applying classifier :', classifier_name, '\n')
    print ('[+]fitting classifier',classifier_name,  'rows:',X_ros.shape[0],'columns:',X_ros.shape[1] ,'labels:',np.unique(y_ros) ,'\n')
    clf = classifier.fit(X_train, y_train)
    print ('[+]predicting classifier', classifier_name, '\n')
    ...
    ax1.set(xlabel='Predicted', ylabel='True', title=classifier_name + " Confusion Matrix") 
    ...
    with open(classifier_name +'results.txt', 'a') as f:
        f.write(classifier_name + ' ' + str(accuracy_score(y_test, y_predict)) + '\n') 
