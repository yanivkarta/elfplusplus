...
print ('[+]fitting gradient boosting classifier', 'rows:',X_ros.shape[0],'columns:',X_ros.shape[1] ,'labels:',np.unique(y_ros) ,'\n')
#plot the dataframe, X_ros, y_ros
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(21, 11))
#run parallel for each classifier in the list:



#clf = ExtraTreesClassifier().fit(X_ros, y_ros)
...
ax1.set(xlabel='Predicted', ylabel='True', title="Random Oversampling Confusion Matrix") 



plot_confusion_matrix(cm, clf.classes_, ax=ax1, title="Random Oversampling Confusion Matrix") 
#classifier, X_train, y_train, X_test, y_test, title, subplot, target_names=None, plot_decision_regions=True, ax=None)
...
#save png
plt.savefig('random_oversampling.png', dpi=300)
plt.tight_layout()
plt.show()
